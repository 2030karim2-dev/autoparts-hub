...
const CountdownBox = ({ value, label }: { value: number; label: string }) => (
  <div className="flex flex-col items-center">
    <span className="bg-primary text-primary-foreground text-sm font-black w-9 h-9 rounded-lg flex items-center justify-center">{String(value).padStart(2, "0")}</span>
      ...
      <div className="px-4 py-4 space-y-5" dir="rtl">
        <section className="animate-fade-in-up stagger-1">
          <div className="bg-gradient-to-l from-destructive/10 to-warning/10 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2"><Flame className="w-5 h-5 text-destructive" /><h2 className="text-base font-black">عروض فلاش</h2></div>
            ...
            <div className="flex gap-2.5 overflow-x-auto scrollbar-hide pb-1">
              {flashDeals.map((item) => (
                <div key={item.id} onClick={() => navigate(`/product/${item.id}`)} className="bg-card rounded-xl shadow-sm p-2.5 min-w-[150px] shrink-0 cursor-pointer active:scale-[0.97] transition-transform">
                  <div className="w-full aspect-square bg-secondary rounded-lg flex items-center justify-center mb-2 relative">
                    ...
                    <span className="absolute top-1 right-1 bg-destructive text-destructive-foreground text-[9px] font-bold px-1.5 py-0.5 rounded">-{item.discount}%</span>
                  </div>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); addItem(item); toast.success("تمت الإضافة"); }} className="w-full bg-primary text-primary-foreground text-[10px] font-bold py-1.5 rounded-lg active:scale-95 transition-transform">أضف للسلة</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="animate-fade-in-up stagger-2">
          <div className="bg-gradient-to-l from-primary/15 to-primary/5 rounded-xl p-4 flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center shrink-0"><Percent className="w-6 h-6 text-primary" /></div>
            ...
            <button onClick={() => { navigator.clipboard.writeText("YEMEN25"); toast.success("تم نسخ الكوبون"); }} className="bg-primary text-primary-foreground text-[11px] font-bold px-3 py-2 rounded-lg active:scale-95 transition-transform shrink-0">نسخ</button>
          </div>
        </section>

        <section className="animate-fade-in-up stagger-3">
          <h2 className="text-base font-bold mb-3 flex items-center gap-1.5"><Percent className="w-4 h-4 text-primary" /> عروض الأسبوع</h2>
          <div className="grid grid-cols-2 gap-2.5">
            {weeklyDeals.map((item) => (
              <div key={item.id} onClick={() => navigate(`/product/${item.id}`)} className="bg-card rounded-xl shadow-sm p-2.5 cursor-pointer active:scale-[0.97] transition-transform">
                <div className="w-full aspect-square bg-secondary rounded-lg flex items-center justify-center mb-2 relative">
                  <img src={item.image} alt={item.name} className="w-3/4 h-3/4 object-contain" />
                  {item.discount && (<span className="absolute top-1 right-1 bg-destructive text-destructive-foreground text-[9px] font-bold px-1.5 py-0.5 rounded">-{item.discount}%</span>)}
                </div>
                <h3 className="text-[11px] font-semibold leading-tight mb-0.5 line-clamp-2">{item.name}</h3>
                <div className="flex items-center gap-0.5 mb-1"><Star className="w-2.5 h-2.5 fill-warning text-warning" /><span className="text-[10px] font-medium">{item.rating}</span></div>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-black">SAR {item.price}</span>

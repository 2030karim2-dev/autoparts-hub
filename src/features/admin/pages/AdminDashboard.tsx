...
const kpis = [
  ...
  { label: "الطلبات الجديدة", value: "128", change: "+8.3%", up: true, icon: ShoppingCart, color: "text-success" },
  { label: "العملاء", value: "1,847", change: "+5.2%", up: true, icon: Users, color: "text-accent" },
...
const categoryData = [
  { name: "فرامل", value: 35, color: "hsl(213, 56%, 24%)" },
  { name: "فلاتر", value: 25, color: "hsl(142, 71%, 45%)" },
  { name: "إنارة", value: 20, color: "hsl(38, 92%, 50%)" },
  { name: "محركات", value: 15, color: "hsl(0, 72%, 51%)" },
  { name: "أخرى", value: 5, color: "hsl(215, 14%, 46%)" },
];
...
const statusColors: Record<string, string> = {
  ...
  "قيد المعالجة": "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400",
  "قيد الشحن": "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400",
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={salesData}>
                ...
                <Bar dataKey="sales" fill="hsl(213, 56%, 24%)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={monthlyData}>
                ...
                <YAxis tick={{ fontSize: 11 }} />
                <Tooltip formatter={(v: number) => `${v.toLocaleString()} ر.س`} />
                <Line type="monotone" dataKey="revenue" stroke="hsl(213, 56%, 24%)" strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-xs">عرض الكل</Badge>
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {recentOrders.map((order) => (
              <div key={order.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-muted/50 transition-colors">

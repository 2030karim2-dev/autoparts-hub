            ...
            return (
              <Card key={ct.type} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setTypeFilter(ct.type)}>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-lg font-bold text-foreground mt-1">{typeCounts[ct.type]}</p>
                  ...
                  {filtered.map((customer) => (
                    <TableRow key={customer.id}>
                      <TableCell>
                        ...
                        <p className="text-[10px] text-muted-foreground">{customer.phone} • {customer.city}</p>
                      </TableCell>
                      <TableCell className="hidden sm:table-cell">
                        <Badge variant="outline" className="text-[10px]">{customer.typeLabel}</Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <span className={`text-[10px] px-2 py-1 rounded-full font-medium ${statusColor[customer.status]}`}>
                          {customer.status}
                        </span>
                      </TableCell>
                      <TableCell>
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setSelected(customer)}>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/50">
                    ...
                    <p className="text-sm font-medium text-foreground" dir="ltr">{selected.phone}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/50">
                  ...
                  <div className="p-3 rounded-lg bg-muted/50">
                    ...
                    <p className="text-sm font-bold text-foreground">{selected.totalOrders}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/50">
                    <p className="text-[10px] text-muted-foreground">الإنفاق</p>
                    <p className="text-sm font-bold text-foreground">{selected.totalSpent.toLocaleString()} ر.س</p>
                  </div>
                </div>

...
interface ReturnRequest {
  ...
  reason: string; status: ReturnStatus; date: string; amount: number;
  adminNote?: string;
}
...
const AdminReturns = () => {
  ...
  const handleAction = (returnId: string, action: "مقبول" | "مرفوض") => {
    setReturns(prev => prev.map(r => r.id === returnId ? { ...r, status: action, adminNote: adminNote || undefined } : r));
    setSelected(null);
    setAdminNote("");
    toast.success(action === "مقبول" ? `تم قبول طلب الإرجاع ${returnId}` : `تم رفض طلب الإرجاع ${returnId}`);
  };
      ...
      <div className="space-y-6">
                ...
                <TableBody>
                  {filtered.length === 0 ? (
                    ...
                    <TableRow key={ret.id}>
                      ...
                      <TableCell><span className={`text-[10px] px-2 py-1 rounded-full font-medium ${statusConfig[ret.status].color}`}>{ret.status}</span></TableCell>
                      <TableCell><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { setSelected(ret); setAdminNote(ret.adminNote || ""); }}><Eye className="h-3.5 w-3.5" /></Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
          <DialogContent className="max-w-md" dir="rtl">
            <DialogHeader><DialogTitle>طلب إرجاع {selected?.id}</DialogTitle></DialogHeader>
            {selected && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/50"><p className="text-[10px] text-muted-foreground">الطلب الأصلي</p><p className="text-sm font-mono font-medium text-foreground">{selected.orderId}</p></div>
                  <div className="p-3 rounded-lg bg-muted/50"><p className="text-[10px] text-muted-foreground">المبلغ</p><p className="text-sm font-bold text-foreground">{selected.amount} ر.س</p></div>
                </div>
                <div className="p-3 rounded-lg bg-muted/50"><p className="text-[10px] text-muted-foreground">المنتج</p><p className="text-sm font-medium text-foreground">{selected.product}</p></div>
                <div className="p-3 rounded-lg bg-muted/50"><p className="text-[10px] text-muted-foreground">سبب الإرجاع</p><p className="text-sm text-foreground">{selected.reason}</p></div>

                {selected.adminNote && selected.status !== "بانتظار المراجعة" && (
                  <div className="p-3 rounded-lg border border-primary/30 bg-primary/5">
                    <p className="text-xs font-semibold text-primary">📝 ملاحظة الإدارة</p>
                    <p className="text-sm text-foreground mt-1">{selected.adminNote}</p>
                  </div>
                )}
                ...
                {selected.status !== "بانتظار المراجعة" && (
                  <div className="text-center p-3 rounded-lg bg-muted/50">
                    <span className={`text-sm px-3 py-1.5 rounded-full font-medium ${statusConfig[selected.status].color}`}>{selected.status}</span>
                  </div>
                )}
              </div>
            )}

...
import {
  ...
  ArrowUpDown, Eye, PackagePlus, PackageMinus, History, Bell
} from "lucide-react";
...
const AdminInventory = () => {
  ...
  const outOfStock = inventoryItems.filter(i => i.status === "نفد المخزون").length;
  const lowStock = inventoryItems.filter(i => i.status === "مخزون منخفض").length;
  const totalValue = inventoryItems.reduce((a, i) => a + i.currentStock, 0);

  const sampleMovements: StockMovement[] = [
    { date: "2026-04-01 14:30", type: "خصم", quantity: 5, note: "طلب ORD-1024" },
    { date: "2026-03-30 10:00", type: "خصم", quantity: 3, note: "طلب ORD-1021" },
    { date: "2026-03-28 09:00", type: "إضافة", quantity: 100, note: "استلام شحنة جديدة" },
    { date: "2026-03-25 16:00", type: "خصم", quantity: 8, note: "طلب ORD-1019" },
    { date: "2026-03-20 11:30", type: "تعديل", quantity: -2, note: "جرد وتعديل يدوي" },
  ];
        ...
        {(outOfStock > 0 || lowStock > 0) && (
          <div className="p-3 rounded-lg border border-destructive/30 bg-destructive/5 flex items-center gap-3">
            ...
            <Button size="sm" variant="outline" className="text-xs shrink-0 gap-1">
              <Bell className="h-3 w-3" /> طلب توريد
            </Button>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-[10px] text-muted-foreground">إجمالي القطع في المخزون</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <TrendingUp className="h-5 w-5 text-green-600 mb-2" />
              <p className="text-lg font-bold text-foreground">{inventoryItems.filter(i => i.status === "متوفر").length}</p>
              <p className="text-[10px] text-muted-foreground">منتجات متوفرة</p>
            </CardContent>
          </Card>
          <Card className={lowStock > 0 ? "border-yellow-300 dark:border-yellow-700" : ""}>
            <CardContent className="p-4">
              <TrendingDown className="h-5 w-5 text-yellow-600 mb-2" />
              <p className="text-lg font-bold text-foreground">{lowStock}</p>
              <p className="text-[10px] text-muted-foreground">مخزون منخفض</p>
            </CardContent>
          </Card>
          <Card className={outOfStock > 0 ? "border-destructive/50" : ""}>
            <CardContent className="p-4">
              <AlertTriangle className="h-5 w-5 text-destructive mb-2" />
              <p className="text-lg font-bold text-foreground">{outOfStock}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="مخزون منخفض">مخزون منخفض</SelectItem>
                  ...
                  {filtered.map((item) => {
                    ...
                    return (
                      <TableRow key={item.id}>
                        <TableCell>
                          <p className="text-sm font-medium text-foreground truncate max-w-[160px]">{item.name}</p>
                          <p className="text-[10px] text-muted-foreground font-mono">{item.sku} • {item.category}</p>
                        </TableCell>
                        <TableCell>
                          <div className="space-y-1">
                            <p className="text-sm font-bold text-foreground">{item.currentStock} <span className="text-[10px] text-muted-foreground font-normal">/ {item.maxStock}</span></p>
                            <Progress value={stockPercent} className="h-1.5 w-20" />
                          </div>
                        </TableCell>
                        <TableCell className="hidden sm:table-cell">
                          {item.daysUntilOut !== null ? (
                            <span className={`text-sm font-medium ${item.daysUntilOut <= 7 ? "text-destructive" : item.daysUntilOut <= 14 ? "text-yellow-600" : "text-foreground"}`}>
                              {item.daysUntilOut} يوم
                            </span>
                          ) : (
                            <span className="text-xs text-destructive font-medium">نفد</span>
                          )}
                        </TableCell>
                        ...
                        <TableCell>
                          <span className={`text-[10px] px-2 py-1 rounded-full font-medium ${statusConfig[item.status].bgColor} ${statusConfig[item.status].color}`}>
                            {item.status}
                          </span>
                        </TableCell>
                        <TableCell>
                          <div className="flex gap-1">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setSelectedItem(item)} title="التفاصيل">
                              <Eye className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setAdjustDialog(item)} title="تعديل الكمية">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-2.5 rounded-lg bg-muted/50 text-center">
                    ...
                    <p className="text-sm font-bold text-foreground">{selectedItem.currentStock}</p>
                  </div>
                  <div className="p-2.5 rounded-lg bg-muted/50 text-center">
                    <p className="text-[10px] text-muted-foreground">الحد الأدنى</p>
                    <p className="text-sm font-bold text-foreground">{selectedItem.minStock}</p>
                  </div>
                  <div className="p-2.5 rounded-lg bg-muted/50 text-center">
                    <p className="text-[10px] text-muted-foreground">الأقصى</p>
                    <p className="text-sm font-bold text-foreground">{selectedItem.maxStock}</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2.5 rounded-lg bg-muted/50">
                    <p className="text-[10px] text-muted-foreground">متوسط المبيعات/شهر</p>
                    <p className="text-sm font-bold text-foreground">{selectedItem.avgMonthlySales} وحدة</p>
                  </div>
                  <div className="p-2.5 rounded-lg bg-muted/50">
                    <p className="text-[10px] text-muted-foreground">آخر توريد</p>
                    <p className="text-sm font-medium text-foreground">{selectedItem.lastRestocked}</p>
                  </div>
                </div>
                ...
                <div>
                  <p className="text-xs font-semibold text-foreground mb-2 flex items-center gap-1.5">
                    <History className="h-3.5 w-3.5" /> سجل الحركة
                  </p>
                  <div className="space-y-2">
                    {sampleMovements.map((m, i) => (
                      <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 text-xs">
                        <span className={`h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-bold ${
                          m.type === "إضافة" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" :
                          m.type === "خصم" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400" :
                          "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
                        }`}>
                          {m.type === "إضافة" ? "+" : m.type === "خصم" ? "-" : "~"}
                        </span>
                        <div className="flex-1 min-w-0">
                          <p className="text-foreground">{m.note}</p>
                          <p className="text-[10px] text-muted-foreground">{m.date}</p>
                        </div>
                        <span className={`font-bold ${m.type === "إضافة" ? "text-green-600" : m.type === "خصم" ? "text-destructive" : "text-blue-600"}`}>
                          {m.type === "إضافة" ? "+" : ""}{m.quantity}
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            {adjustDialog && (
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-muted/50 text-center">
                  <p className="text-sm font-bold text-foreground">{adjustDialog.name}</p>
                  <p className="text-xs text-muted-foreground">المخزون الحالي: {adjustDialog.currentStock}</p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <Button variant="outline" className="gap-1.5 h-12 flex-col" onClick={() => {}}>
                    <PackagePlus className="h-5 w-5 text-green-600" />
                    <span className="text-[10px]">إضافة مخزون</span>
                  </Button>
                  <Button variant="outline" className="gap-1.5 h-12 flex-col" onClick={() => {}}>
                    <PackageMinus className="h-5 w-5 text-destructive" />
                    <span className="text-[10px]">خصم مخزون</span>
                  </Button>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="سبب التعديل..." />
                </div>
                <Button className="w-full" onClick={() => setAdjustDialog(null)}>تأكيد التعديل</Button>
              </div>
            )}
          </DialogContent>
        </Dialog>

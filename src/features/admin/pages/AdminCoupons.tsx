...
import { Plus, Tag, Percent, Copy, Trash2, Edit, TicketPercent, Calendar, Users } from "lucide-react";
              ...
              <div className="space-y-4 pt-2">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input placeholder="BRAKE20" className="font-mono uppercase" dir="ltr" />
                  </div>
                  <div className="space-y-1.5">
                ...
                <Button className="w-full" onClick={() => setDialogOpen(false)}>إنشاء الكوبون</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card>
            <CardContent className="p-3 text-center">
              <TicketPercent className="h-5 w-5 text-primary mx-auto mb-1" />
              <p className="text-lg font-bold text-foreground">{activeCoupons}</p>
              <p className="text-[10px] text-muted-foreground">كوبونات نشطة</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-3 text-center">
          ...
          <Card>
            <CardContent className="p-3 text-center">
              ...
              <p className="text-lg font-bold text-foreground">{coupons.filter(c => new Date(c.expiresAt) < new Date()).length}</p>
              <p className="text-[10px] text-muted-foreground">منتهية الصلاحية</p>
            </CardContent>
          </Card>
        </div>
                    ...
                    <TableRow key={coupon.id}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-sm font-mono font-bold text-foreground" dir="ltr">{coupon.code}</p>
                            <p className="text-[10px] text-muted-foreground">حتى {coupon.expiresAt}</p>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        ...
                        {coupon.minOrder > 0 && (
                          <p className="text-[10px] text-muted-foreground">فوق {coupon.minOrder} ر.س</p>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="hidden md:table-cell">
                        <Badge variant="outline" className="text-[10px]">{coupon.targetLabel}</Badge>
                      </TableCell>
                      <TableCell>
                        <Badge className={`text-[10px] ${coupon.active ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-muted text-muted-foreground"}`}>
                          {coupon.active ? "نشط" : "معطل"}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" className="h-7 w-7" title="نسخ الكود">
